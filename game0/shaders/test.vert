...
void main()
{
	...
	float tt = float(itime) * PI * float(.001);
	mat3 test_trans = mat3(cos(tt),sin(tt),0,  -sin(tt),cos(tt),0,  0,0,1) * float(1 + sin(tt) * .5);
	vec3 out_pos = vec3(ipmat * mat4(test_trans) * vec4(pos,1));
