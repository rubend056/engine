...
#define PREFAB_VAR_DEF(type, name) \
	type name;                     \
	bool PREFAB_VAR_DIRTY_NAME(name) = false;
...
#define PREFAB_VAR_CHECK_DEF(type, name)                     \
	void check_##name() {                                    \
		if (ref)                                             \
			PREFAB_VAR_DIRTY_NAME(name) = ref->name == name; \
		else {                                               \
...
class PREFAB_NAME {
   ...
   protected:
	...
	enum class DIRTY_ENUM { First = -1,
							CLASSNAME_EXPANSION(PREFAB_VAR_ENUM_DIRTY) Last };
   ...
   public:
	...
	void serialize(Archive& ar) {
		...
		if (ref) {
			CLASSNAME_EXPANSION(PREFAB_VAR_ADD_DIRTY)
		} else {
			for (int i = 0; i < (int)DIRTY_ENUM::Last; ++i) dirty.push_back(true);
		}
		...
		if (count != dirty.size()) {
			printf(
				ANSI_COLOR_RED "Warning, count is different: %d in file, %d in class" ANSI_COLOR_RESET "\n", count, (int)dirty.size());
...
#define PREFAB_VAR_SERIALIZE_VARS(type, name)                              \
	if (i++ < count && dirty[(int)DIRTY_ENUM::PREFAB_VAR_ENUM_NAME(name)]) \
	PREFAB_VAR_SERIALIZE(type, name)
