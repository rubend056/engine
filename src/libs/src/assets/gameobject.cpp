...
std::shared_ptr<Referentiable> GameObject::foster(const std::shared_ptr<Referentiable>& child) {
	...
	if (auto go = std::dynamic_pointer_cast<GameObject>(child))
		Parent::foster(child);
	else if (auto comp = std::dynamic_pointer_cast<Component>(child)) {
		comp->parent = this;
	} else
		throw(std::string("Not go or comp"));
	return child;
}
...
void GameObject::add(const std::shared_ptr<Component>& _comp) {
	...
	if (comp->max_num() > 0 && get_comps(t_name).size() >= comp->max_num()) {
		printf(ANSI_COLOR_YELLOW "Max num %d for %s" ANSI_COLOR_RESET ENDL, comp->max_num(), t_name.c_str());
		return;
	}
...
void GameObject::imgui_draw() {
	...
	if (refs.size()) {
		auto comp = assets::get_file<Component>(refs);
		if (comp) {
			add(comp);
		}
	}

	if (auto comp = menus::select_asset<Component>("add_component_popup", Factory_Camera, MenuSelectFlags_NewOnly)) {
		add(comp);
	}
}