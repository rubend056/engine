...
// FUNCTIONS
bool Mesh::supported(std::string ext) { return importer.IsExtensionSupported(ext); }
void Mesh::load() {
	loaded = false;
	...
	if (!supported(ext)) {
		std::cerr << "Extension not supported " << ext << std::endl;
		return;
	}
	...
	if (scene->HasMeshes()) {
		...
		size_t i = 0;
		auto set_data = [&i](size_t size, void* data) {glBufferSubData(GL_ARRAY_BUFFER, i, size, data);i+=size; };
		for (uint i = 0; i < scene->mNumMeshes; ++i) {
			auto& mesh = scene->mMeshes[i];
			if (!mesh->HasPositions())
				continue;
		...
		printf(ANSI(YELLOW, "No meshes found, din't read anything") ENDL);
	}
...
void Mesh::imgui_draw() {
	...
	ImGui::TextDisabled("%zu VAO's", vaos.size());
	for (auto& vao : vaos) {
