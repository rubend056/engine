...
unsigned int Referentiable::get_id() {
	if (!_id)
		gen();
	return _id;
};
...
void Referentiable::gen(unsigned int wanted_id) {
	...
	if (!max_it) {
		printf(ANSI_COLOR_RED "ERROR max_it == 0" ANSI_COLOR_RESET ENDL);
		return;
	}
	if (!_id) {
		printf(ANSI_COLOR_RED "id is zero" ANSI_COLOR_RESET ENDL);
		return;
	}
	// Insert the found id
	used_ids.insert(_id);
...
void Referentiable::_my_ref(std::vector<unsigned int>& v) {
	if (parent)
		parent->_my_ref(v);	 // If I have a parent, push back parent id
...
std::shared_ptr<Referentiable> Parent::foster(const std::shared_ptr<Referentiable>& child) {
	if (!child || (child && (child.get() == this || child->parent == this)))
